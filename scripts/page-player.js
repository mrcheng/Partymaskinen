...
function PagePlayer() {
	...
	this.events = {
		...
		finish: function () {
			...
			//  		}
			//  		else if (!self.config.playNext) {
			//  			self.setPageTitle();
			//  			self.resetPageIcon();
			//  		}
			//  		else if (self.config.repeatPlaylist) {
			//  			// i`ve add own config setting repeatPlaylist and turn it True
			//  			// at end of list, and playNext is on
			//  			pl.handleClick({ target: pl.links[0] }); // play first link
			//  		}
		},
		//  	
		//    finish: function() {
	...
	this.playLink = function () {
		...
		if (!o.href || (!sm.canPlayLink(o) && !self.hasClass(o, 'playable')) || self.hasClass(o, 'exclude')) {
			...
			if (thisSound) {
				...
				if (thisSound._lastURL === soundURL) {
